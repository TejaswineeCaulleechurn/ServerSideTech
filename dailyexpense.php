...
<head>
	...
	<title>MoneyManager - Daily Expense Report</title>
	...
	<!-- bootstrap class to generate the panel for expense list -->
		<div class="col-10 col-offset-1 col-sm-8 col-sm-offset-2 col-md-4 col-md-offset-4">
			...
			<!-- panel heading to indicate the purpose of the page -->
				<div class="panel-heading loginpanel_heading h4" align="center">Daily Expense Report</div>
				<!-- panel body to contain the contents of the page -->
					...
					.		<?php
							...
							?>
							<!-- list header -->
							<h5 align="center" style="color:blue">Daily Expense Report for <?php echo $currentdate?></h5>
							<hr />
								...
								$ret=mysqli_query($con,"SELECT month(ExpenseDate) as expmonth,year(ExpenseDate) as expyear, ExpenseCost, ExpenseDate, ExpenseItem FROM tb_expenses  where (ExpenseDate = '$currentdate') && (UserId='$userid') order by ExpenseDate");
								...
								$monthlyincome=mysqli_query($con,"SELECT month(IncomeDate) as incmonth,year(IncomeDate) as incyear, IncomeAmount, IncomeDate, IncomeDetails FROM tb_income  where (IncomeDate = '$currentdate') && (UserId='$userid') order by IncomeDate");
								/* variable count for each line retrieved - ID of each line */
								$cnt=1;
